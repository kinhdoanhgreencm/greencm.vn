...
const CarSales: React.FC = () => {
  ...
  return (
    <div className="pt-20 bg-gray-50 min-h-screen">
        ...
        <div className="relative z-10 container mx-auto px-4 text-center">
          ...
          <div className="flex flex-col md:flex-row gap-6 justify-center mt-10">
            <button 
              ...
              className={`relative overflow-hidden group p-8 rounded-3xl w-full md:w-80 text-left transition-all duration-300 border-2 ${activeTab === 'family' ? 'bg-gcm-green border-gcm-green scale-105 shadow-[0_0_30px_rgba(0,210,106,0.4)]' : 'bg-black/50 border-white/20 hover:bg-black/70'}`}
            >
              <div className="relative z-10">
                <Users size={32} className={`mb-4 ${activeTab === 'family' ? 'text-black' : 'text-white'}`} />
                <h3 className={`text-2xl font-bold ${activeTab === 'family' ? 'text-black' : 'text-white'}`}>Dành Cho Gia Đình</h3>
                <p className={`text-sm mt-2 font-medium ${activeTab === 'family' ? 'text-black/80' : 'text-gray-400'}`}>Sống Xanh - Sống Đẳng Cấp</p>
              </div>
            </button>

            <button 
              onClick={() => setActiveTab('service')}
              className={`relative overflow-hidden group p-8 rounded-3xl w-full md:w-80 text-left transition-all duration-300 border-2 ${activeTab === 'service' ? 'bg-gcm-green border-gcm-green scale-105 shadow-[0_0_30px_rgba(0,210,106,0.4)]' : 'bg-black/50 border-white/20 hover:bg-black/70'}`}
            >
              <div className="relative z-10">
                <Truck size={32} className={`mb-4 ${activeTab === 'service' ? 'text-black' : 'text-white'}`} />
                <h3 className={`text-2xl font-bold ${activeTab === 'service' ? 'text-black' : 'text-white'}`}>Vận Tải & Dịch Vụ</h3>
                <p className={`text-sm mt-2 font-medium ${activeTab === 'service' ? 'text-black/80' : 'text-gray-400'}`}>Giải Pháp Kinh Doanh Tối Ưu</p>
              </div>
            </button>
          </div>
        </div>
      ...
      {/* 2. FAMILY ZONE */}
      {activeTab === 'family' && (
        <section className="py-20 container mx-auto px-4 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {NEW_CARS_FAMILY.map((car) => (
              <div key={car.id} className="bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 group flex flex-col">
          ...
          <div className="container mx-auto px-4">
             ...
             <div className="mb-20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   {NEW_CARS_SERVICE.map((car) => (
                      <div key={car.id} className="bg-white rounded-3xl p-6 shadow-md border border-gray-100 flex flex-col md:flex-row gap-6 hover:border-gcm-green transition-all">
